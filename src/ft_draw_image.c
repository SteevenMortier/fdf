...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/03 11:12:16 by smortier          #+#    #+#             */
/*   Updated: 2018/03/03 11:12:19 by smortier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_draw_image(t_params *param)
{
	...
	param->img.data = (int *)mlx_get_data_addr(param->img.img_ptr,
										&param->img.bpp, &param->img.size_l,
										&param->img.endian);
	while (++y_img < HIGHT)
	{
		x_img = -1;
		while (++x_img < WIDTH)
		{
			if (param->bres_tab[x_img][y_img])
				param->img.data[y_img * WIDTH + x_img] =
						param->bres_tab[x_img][y_img];
			else
